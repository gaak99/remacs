...
/*gb static Lisp_Object file_attributes (int, char const *, Lisp_Object, */
/* 				    Lisp_Object, Lisp_Object); */

Lisp_Object file_attributes_c_internal (char const *name,
					Lisp_Object dirname,
					Lisp_Object filename,
					Lisp_Object id_format);
  ...
  if (S_ISLNK (s.st_mode))
    {
	 ...
	 is also a symlink.  */
      ...
      if (NILP (file_type)) {
	report_file_error ("emacs_readlinkat fail: filename", filename);
	return unbind_to (count, Qnil);
      }
    }
  else
